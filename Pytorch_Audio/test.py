...
def custom_test():
    ...
    with torch.no_grad():
        ...
        for item in custom_test_loader:
            images = item['image'].to(device)
            outputs = custom_model(images)
            _, predicted = torch.max(outputs.data, 1)
            #print('예측값: ', str(classes[predicted[0]]))
            if int(predicted[0])==0:
                print('Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog Dog ')
            #print(str(classes[int(predicted[0])]))
...
custom_model.load_state_dict(torch.load('./model/second_model.pth', map_location=lambda storage, loc: storage))
